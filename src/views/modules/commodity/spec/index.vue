 ...
 * @LastEditTime: 2021-03-03 11:15:01
-->
<template>
  <el-dialog width="50%" v-model="visible" :title="`规格列表`" :close-on-click-modal="false" :destroy-on-close="true" @closed="dialogClosedHandle">
    <el-form ref="formRef" :inline="true" :model="form" @keyup.enter="getList()">
      <el-form-item prop="name">
        <el-input v-model="form.name" placeholder="规格名称" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button @click="getList()">查询</el-button>
        <el-button @click="clearJson(form), getList()">重置</el-button>
        <el-button v-if="isAuth('sys:goodsspec:save')" type="primary" @click="addOrEditHandle()">新增</el-button>
        <el-button v-if="isAuth('sys:goodsspec:delete')" type="danger" @click="delHandle()" :disabled="selection.length <= 0"> 批量删除 </el-button>
      </el-form-item>
    </el-form>
    <el-table class="base-table" :data="list" border @selection-change="selectionHandle">
      ...
      <el-table-column header-align="center" align="center" label="ID" prop="id" width="80"></el-table-column>
      <el-table-column header-align="center" align="center" label="规格名称" prop="name"></el-table-column>
      ...
      <el-table-column header-align="center" align="center" label="操作" width="90" fixed="right">
        <template v-slot="scope">
          <el-button v-if="isAuth('sys:goodsspec:update')" type="text" size="small" @click="addOrEditHandle(scope.row.id)">修改</el-button>
          <el-button v-if="isAuth('sys:goodsspec:delete')" type="text" size="small" @click="delHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
import { pageList, del } from '@API/commodity/spec/index'
