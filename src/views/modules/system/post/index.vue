<template>
  <Container :class="n.b()">
    <template #headbar>
      <el-form inline @keyup.enter="onEnterKeyup">
        ...
        <el-form-item>
          ...
          <el-button v-permission="'post:create'" type="primary" @click="onAddOrEdit()">新增</el-button>
          <el-button v-permission="'post:delete'" type="danger" :disabled="!selection.length" @click="onDelete()">批量删除</el-button>
          <el-button v-permission="'post:export'" v-repeat @click="onExport">导出</el-button>
        </el-form-item>
      </el-form>
    </template>
    <el-table v-loading="loading" :data="list" border @selection-change="onSelectionChange">
      ...
      <el-table-column v-permission="'post:update|post:delete'" align="center" label="操作" width="110" fixed="right">
        <template v-slot="{ row }">
          <div class="f_jc-center">
            <el-button v-permission="'post:update'" type="primary" link @click="onAddOrEdit(row.id)">编辑</el-button>
            <el-button v-permission="'post:delete'" type="danger" link @click="onDelete(row.id)">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
import { postPageApi, postDeleteApi, postSetStatusApi, postExportApi} from '@/apis'

const n = useNamespace('post')
...
const onStatusChange = (row) => {
  ...
  postSetStatusApi(params).then(r => {
    if (r) {
      ElMessage.success('操作成功!')
    } else {
...
const onExport = () => {
  postExportApi({ ...form }).then(r => r && download(r.data, '', 'xlsx', r.type))
}
