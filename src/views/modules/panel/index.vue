...
import { pageApi, deleteApi, setStatusApi, exportApi } from '@/api/panel'
  ...
  <Container>
    <template #header>
      <el-form ref="refForm" :inline="true" @keyup.enter="reacquireHandle()">
        ...
        <el-form-item>
          <DictSelect v-model="form.type" code="PANEL" placeholder="类型" />
        </el-form-item>
        <el-form-item>
          <DictSelect v-model="form.type" code="PLATFORM" placeholder="平台" />
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          ...
          <el-button
            v-permission="'panel:create'"
            type="primary"
            @click="addEditHandle()">新增</el-button>
          <el-button
            v-permission="'panel:delete'"
            type="danger"
            @click="deleteHandle()"
            :disabled="selection.length <= 0">批量删除</el-button>
          <el-button v-repeat v-permission="'panel:export'" @click="exportHandle">导出</el-button>
        </el-form-item>
      </el-form>
    </template>
    <template #default>
      <el-table
        ...
        <el-table-column
          ...
          prop="platform">
          <template v-slot="{ row }">
            <el-tag :type="row.platform_type">{{ row.platform_dict }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          align="center"
        ...
        <el-table-column
          ...
          show-overflow-tooltip />
        <el-table-column
          v-permission="'panel:update|panel:delete'"
          align="center"
          label="操作"
          width="110"
          fixed="right">
