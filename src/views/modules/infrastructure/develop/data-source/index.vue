<template>
  <Container :class="n.b()">
    ...
    <el-table v-loading="loading" :data="list" border @selection-change="onSelectionChange">
      ...
      <el-table-column align="center" label="驱动名称" prop="driverClassName" show-overflow-tooltip />
      <el-table-column align="center" label="连接" prop="url" show-overflow-tooltip />
      ...
      <el-table-column v-permission="'dataSource:update|dataSource:delete'" align="center" label="操作" width="110" fixed="right">
        <template v-slot="{ row }">
          <div class="f_jc-center">
            <el-button v-permission="'dataSource:update'" type="primary" link @click="onAddOrEdit(row.id)">编辑</el-button>
            <el-button v-permission="'dataSource:delete'" type="danger" link @click="onDelete(row.id)">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
import { dataSourcePageApi, dataSourceDeleteApi, dataSourceSetStatusApi, dataSourceExportApi} from '@/apis'

const n = useNamespace('data-source')
...
const getData = () => {
  ...
  dataSourcePageApi(params).then(r => {
    if (r) {
...
const onDelete = (id) => {
  ...
  ElMessageBox.confirm(
    ...
    dataSourceDeleteApi({ keys: ids }).then(r => {
      if (r) {
...
const onStatusChange = (row) => {
  ...
  dataSourceSetStatusApi(params).then(r => {
    if (r) {
      ElMessage.success('操作成功!')
      getData()
    } else {
...
const onExport = () => {
  dataSourceExportApi({ ...form }).then(r => r && download(r.data, '', 'xlsx', r.type))
}
