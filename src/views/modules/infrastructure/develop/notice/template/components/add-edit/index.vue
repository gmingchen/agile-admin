  ...
  <el-dialog
    ...
    <el-form ref="formRef" v-loading="loading" :model="form" :rules="rules" @keyup.enter="submit()" label-position="top">
      ...
      <el-form-item label="模版内容" prop="content">
        <el-input v-model="form.content" placeholder="模版内容，变量使用 ${name} 表示" type="textarea" :rows="4" />
      </el-form-item>
...
import { noticeTemplateInfoApi, noticeTemplateCreateApi, noticeTemplateUpdateApi } from '@/apis'
...
const onConfirm = () => {
  formRef.value.validate(async valid => {
    if (valid) {
      loading.value = true
      const r = await (form.id ? noticeTemplateUpdateApi(form) : noticeTemplateCreateApi(form))
      if (r) {
        visible.value = false
...
const open = (id) => {
  ...
  if (id) {
    loading.value = true
    form.id = id
    noticeTemplateInfoApi({ id }).then(r => {
      if (r) {
