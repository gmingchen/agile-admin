<template>
  <Container :class="n.b()">
    <template #headbar>
      <el-form inline @keyup.enter="onEnterKeyup">
        ...
        <el-form-item>
          ...
          <el-button v-permission="'noticeTemplate:create'" type="primary" @click="onAddOrEdit()">新增</el-button>
          <el-button v-permission="'noticeTemplate:delete'" type="danger" :disabled="!selection.length" @click="onDelete()">批量删除</el-button>
          <el-button v-permission="'noticeTemplate:export'" v-repeat @click="onExport">导出</el-button>
        </el-form-item>
      </el-form>
    </template>
    <el-table v-loading="loading" :data="list" border @selection-change="onSelectionChange">
      ...
      <el-table-column align="center" label="模版内容" prop="content" show-overflow-tooltip />
      <el-table-column align="center" label="备注" prop="remark" show-overflow-tooltip />
      <el-table-column align="center" label="状态" prop="status" width="80">
        <template v-slot="{row}">
          <el-switch
            v-permission="'noticeTemplate:status'"
      ...
      <el-table-column v-permission="'noticeTemplate:update|noticeTemplate:delete'" align="center" label="操作" width="110" fixed="right">
        <template v-slot="{ row }">
          <div class="f_jc-center">
            <el-button v-permission="'noticeTemplate:update'" type="primary" link @click="onAddOrEdit(row.id)">编辑</el-button>
            <el-button v-permission="'noticeTemplate:delete'" type="danger" link @click="onDelete(row.id)">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
import { noticeTemplatePageApi, noticeTemplateDeleteApi, noticeTemplateSetStatusApi, noticeTemplateExportApi} from '@/apis'

const n = useNamespace('notice-template')
...
const getData = () => {
  ...
  noticeTemplatePageApi(params).then(r => {
    if (r) {
...
const onDelete = (id) => {
  ...
  ElMessageBox.confirm(
    ...
    noticeTemplateDeleteApi({ keys: ids }).then(r => {
      if (r) {
...
const onStatusChange = (row) => {
  ...
  noticeTemplateSetStatusApi(params).then(r => {
    if (r) {
      ElMessage.success('操作成功!')
    } else {
...
const onExport = () => {
  noticeTemplateExportApi({ ...form }).then(r => r && download(r.data, '', 'xlsx', r.type))
}
...
$prefix: notice-template#{$element-separator};
@include b(notice-template) {}
</style>
