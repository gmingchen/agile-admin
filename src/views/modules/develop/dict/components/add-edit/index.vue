  ...
  <el-dialog
    ...
    <el-form ref="formRef" v-loading="loading" :model="form" :rules="rules" @keyup.enter="submit()" label-position="top">
      <el-form-item label="编码" prop="code">
        <el-input v-model="form.code" placeholder="编码" maxlength="32" show-word-limit />
      </el-form-item>
...
const onConfirm = () => {
  formRef.value.validate(async valid => {
    if (valid) {
      loading.value = true
      const r = await form.id ? dictUpdateApi(form) : dictCreateApi(form)
      if (r) {
        visible.value = false
